...
import { IMatchesRepository, IRequest, IRequestUpdate } from '../IMatchesRepository';
...
export default class MatchesRepository implements IMatchesRepository {
  ...
  async finallyMatch(id: number): Promise<void> {
    await this._repository.update({ inProgress: 0 }, { where: { id } });
  }

  async updateMatch(id: number, { homeTeamGoals, awayTeamGoals }: IRequestUpdate): Promise<void> {
    await this._repository.update({ homeTeamGoals, awayTeamGoals }, { where: { id } });
  }
